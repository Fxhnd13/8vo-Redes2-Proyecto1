...
#de banda por mac
DEV1=ens8 #Interfaz de debian que compartirá el enlace con los clientes 
#DEV2=ens3
...
filter_mac() {
    ...
    tc filter add dev $DEV1 parent 1: protocol ip prio 5 u32 match u16 0x0800 0xFFFF at -2 match u16 0x${M2} 0xFFFF at -4 match u32 0x${M0}${M1} 0xFFFFFFFF at -8 flowid $2 #matcheamos la mac si es origen
    tc filter add dev $DEV1 parent 1: protocol ip prio 5 u32 match u16 0x0800 0xFFFF at -2 match u32 0x${M1}${M2} 0xFFFFFFFF at -12 match u16 0x${M0} 0xFFFF at -14 flowid $2 #matcheamos la mac si es destino, probablemente esto no sea útil
}

tc class add dev $DEV1 parent 1:1 classid 1:10 htb rate 1Kbit
tc class add dev $DEV1 parent 1:1 classid 1:11 htb rate 1Kbit
tc class add dev $DEV1 parent 1:1 classid 1:12 htb rate 1Kbit
tc class add dev $DEV1 parent 1:1 classid 1:13 htb rate 1Kbit
#tc class add dev $DEV2 parent 1:1 classid 1:10 htb rate 1Kbit
#tc class add dev $DEV2 parent 1:1 classid 1:11 htb rate 1Kbit
#tc class add dev $DEV2 parent 1:1 classid 1:12 htb rate 1Kbit
#tc class add dev $DEV2 parent 1:1 classid 1:13 htb rate 1Kbit
